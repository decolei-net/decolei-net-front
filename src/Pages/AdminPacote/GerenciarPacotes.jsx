...
export default function GerenciarPacotes() {
  ...
  const handleDelete = async (pacote) => {
    ...
    if (
      window.confirm(
        `Tem certeza que deseja excluir o pacote "${pacote.titulo}"? Esta ação não pode ser desfeita.`,
      )
    ) {
      try {
  ...
  return (
    <div className="p-2 sm:p-4">
      ...
      <div className="bg-white p-4 rounded-lg shadow-md overflow-x-auto">
            ...
            {pacotes.map((pacote) => {
              ...
              return (
                <tr key={pacote.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {pacote.vagasDisponiveis} / {pacote.quantidadeVagas}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                    <button
                      onClick={() => navigate(`/dashboard-admin/pacotes/editar/${pacote.id}`)}
        ...
        {pacotes.length === 0 && (
          <p className="text-center py-4 text-gray-500">Nenhum pacote cadastrado.</p>
        )}
      </div>
    </div>
  );
