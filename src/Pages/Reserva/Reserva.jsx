...
export default function Reserva() {
    ...
    if (loading) return <div className="text-center p-10 font-bold text-gray-500" role="status">Carregando detalhes...</div>;
    if (error) return <div className="text-center p-10 text-red-500 font-bold" role="alert">{error}</div>;
            ...
            <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg">
                <section aria-labelledby="detalhes-pacote-heading">
                    <div className="border-b pb-6 mb-6">
                        <h2 id="detalhes-pacote-heading" className="text-2xl font-bold text-blue-600">{pacote.titulo}</h2>
                        <p className="text-sm text-gray-500 mt-1">
                            De <time dateTime={pacote.dataInicio}>{new Date(pacote.dataInicio).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}</time> até <time dateTime={pacote.dataFim}>{new Date(pacote.dataFim).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}</time>
                        </p>
                    </div>
                </section>

                <section aria-labelledby="viajantes-heading">
                    <h3 id="viajantes-heading" className="text-xl font-bold mb-2">Viajantes</h3>
                    ...
                    {viajantes.map((viajante, index) => (
                        <fieldset key={index} className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 items-center border-t pt-4">
                            <legend className="sr-only">Informações do Acompanhante {index + 1}</legend>
                            <div>
                                <label htmlFor={`nome-acompanhante-${index}`} className="sr-only">Nome completo do Acompanhante {index + 1}</label>
                                <input id={`nome-acompanhante-${index}`} name="nome" type="text" value={viajante.nome} onChange={(e) => handleViajanteChange(index, e)} placeholder={`Nome completo do Acompanhante ${index + 1}`} className="p-3 border rounded-md w-full"/>
                            </div>
                            <div className="flex items-center gap-2">
                                <label htmlFor={`documento-acompanhante-${index}`} className="sr-only">Documento (CPF) do Acompanhante {index + 1}</label>
                                <input
                ...
                <div className="border-t mt-8 pt-6">
                    ...
                    {error && <p className="text-red-500 text-center mt-4 text-sm" role="alert">{error}</p>}
                </div>
            </div>
        </div>
    );
