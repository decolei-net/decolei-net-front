...
export default function GerenciarPacotes() {
    ...
    // A função de busca agora é executada automaticamente quando os filtros mudam
    useEffect(() => {
        const fetchPacotes = async () => {
            try {
                setLoading(true);
                // A validação de data aqui evita requisições desnecessárias e erros
                ...
                const data = await pacoteService.getPacotes(filtrosFormatados);
                setPacotes(data);
        ...
        // Adicione um pequeno delay para evitar muitas requisições rápidas (debounce)
        const debounceTimer = setTimeout(() => {
            fetchPacotes();
        }, 500); // Espera 500ms antes de fazer a requisição

        // Limpa o timer para evitar que a requisição anterior seja executada
        return () => clearTimeout(debounceTimer);
    }, [filtros]); // O array de dependências faz com que este efeito rode sempre que 'filtros' mudar
    ...
    return (
        <div className="p-2 sm:p-4">
            ...
            <div className="bg-white rounded-lg shadow p-4 mb-6">
                {/* Remova a tag <form> e o botão de submit */}
